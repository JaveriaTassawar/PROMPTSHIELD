...
#### ✅ Task 31 — Run evaluation and check against targets — **done** (Faiqa)

_Metrics generated by running `evaluate.py --output`, not transcribed — committed as [`results/metrics_real.json`](results/metrics_real.json) and [`results/metrics_synthetic.json`](results/metrics_synthetic.json). Write-up in [`TASK31_RESULTS.md`](TASK31_RESULTS.md)._

_**TARGET CHECK — both pass on the real-world held-out set:**_

| Target (SRS §1.4) | Required | Achieved | |
|---|---:|---:|---|
| Accuracy | > 90% | **99.11%** | **PASS** |
| Weighted F1 | > 0.85 | **0.9911** | **PASS** |

_**The section 4 fallback (≥80%, SRS §2.5) is NOT invoked.** Nothing to disclose to the advisor on that front, and `MODEL_CARD.md` should say the primary target was met outright._

_**Verified independently, not taken on trust.** Three checks: the full suite passes (66 passed, 6 skipped, 0 failed); the committed confusion matrix sums to 42,309/42,690 = 0.99107…, matching the stated accuracy to 9 decimals, which hand-typed numbers do not do; and re-running `evaluate.py` on a 600-row stratified sample of `test.parquet` (200 per class, chosen independently) reproduced the same per-sub-type ranking — Persona Hijacking worst at 75.0% on 8 rows, Web Content Injection perfect._

_**TWO FIGURES FOR `MODEL_CARD.md` (Task 32) that are in the JSON but not in her write-up:**_

_**1. Macro F1 = 0.9756**, against weighted F1 0.9911. Macro counts each class equally; weighted counts by size, so 27,001 near-perfect Indirect rows dilute Direct Jailbreak's 0.9420 F1. Both are honest, and macro still clears the 0.85 target comfortably — quote it in 32.4 rather than wait to be asked for it._

| Class | F1 | Support |
|---|---:|---:|
| Indirect Injection | 0.9996 | 27,001 |
| Safe | 0.9853 | 12,570 |
| **Direct Jailbreak** | **0.9420** | 3,119 |

_**2. The dominant error is a false positive, not a miss.** Of 381 total errors, **244 are Safe prompts classified as Direct Jailbreak — 64% of everything the model gets wrong.**_

```
              predicted →
actual ↓      Safe   Direct   Indirect
Safe         12316      244         10
Direct         113     3006          0
Indirect         1       13      26987
```

_Only 1.9% of Safe rows, so it is not alarming — but it is the failure an examiner would actually trigger by typing a harmless prompt in the live demo, and it is worth naming in 32.5 before someone finds it. Direct Jailbreak's 0.9212 precision is the same fact seen from the other side._
...
>
> **32.5 has three things to disclose**, all measured rather than guessed: Persona Hijacking **84.40%** (34 errors in 218) · Role Override **88.89%** on only **18** rows, a caveat rather than a score · **244 Safe prompts flagged as Direct Jailbreak**, 64% of all errors.
>
> **32.6 is a no-op** — the >90% / F1 >0.85 target was met outright in Task 31, so the §2.5 fallback wording is not needed. Say the primary target was met.
