...
**Dataset Prep**
🔵 Task 13 — Create `ml/data/raw/`, `ml/data/processed/`, `ml/preprocessing/`, `ml/training/`, `ml/inference/` + `requirements.txt`
🟡 Task 14 — `download_datasets.py` — copy/fetch all 20 sources into `ml/data/raw/` (all already downloaded to `../dataset/`); re-runnable, prints a summary
🟡 Task 15 — `inspect_sources.py` — row count, columns, 2 examples per source; confirm each matches the inventory
🟡 Task 16 — `map_labels.py` — one adapter per source → `{text, label_3class, subtype, source}`, using the 3-class / 8-sub-type taxonomy
